#pragma checksum "E:\Ashish\User Autorization Excel Enc 21-11-22\Admin\LaserProgramming.aspx.cs" "{ff1816ec-aa5e-4d10-87f7-6f4963833460}" "A0181D6CF7D9DF74C1419C39DF49245C10F6FB5F"

#line 1 "E:\Ashish\User Autorization Excel Enc 21-11-22\Admin\LaserProgramming.aspx.cs"
using System;
...
public partial class Admin_LaserProgramming : System.Web.UI.Page
{
                ...
                using (SqlCommand cmd = con.CreateCommand())
                {
                    ...
                    foreach (DataRow row in dt.Rows)
                    {
                        ...
                        else
                        {
                            ...
                            SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblLaserPrograming] SET [InwardQty] = '" + inwardqy.ToString() + "', [OutwardQty] = '" + totoutward.ToString() + "',UpdatedDate='" + UpdatedDate + "' WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                            cmdupdate.ExecuteNonQuery();
                        }
                        con.Close();
                    }
    ...
    protected void dgvLaserprogram_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            gvDetails.DataSource = GetData(string.Format("select * from vw_LaserProgramming where LaserProgId='{0}'", Id));
            gvDetails.DataBind();
        }
    }
