#pragma checksum "E:\Ashish\User Autorization Excel Enc 21-11-22\Admin\PowderCoating.aspx.cs" "{ff1816ec-aa5e-4d10-87f7-6f4963833460}" "5A3468DE91F140FE8B03557663A425C7446DA77B"

#line 1 "E:\Ashish\User Autorization Excel Enc 21-11-22\Admin\PowderCoating.aspx.cs"
...
public partial class Admin_PowderCoating : System.Web.UI.Page
{
                ...
                using (SqlCommand cmd = con.CreateCommand())
                {
                    ...
                    foreach (DataRow row in dt.Rows)
                    {
                        ...
                        else
                        {
                            ...
                            SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblFinalAssembly] SET [InwardQty] = '" + totOutwardqnt.ToString() + "',[InwardDtTime]='" + row["outwarddatetime"].ToString() + "' WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                            cmdupdate.ExecuteNonQuery();
                        }

                        if (row["inwardqty"].ToString() == row["outwardqty"].ToString())
                        {
                            SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblPowderCoating] SET [OutwardQty] = '" + row["totalinward"].ToString() + "',[InwardQty]='0',[IsComplete]=1,UpdatedDate='" + UpdatedDate + "' WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                            cmdupdate.ExecuteNonQuery();
                        }
                        else
                        {
                            ...
                            SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblPowderCoating] SET [InwardQty] = '" + inwardqy.ToString() + "', [OutwardQty] = '" + totoutward.ToString() + "',UpdatedDate='" + UpdatedDate + "' WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                            cmdupdate.ExecuteNonQuery();
                        }
                        con.Close();
                    }
    ...
    protected void dgvPowderCoating_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            gvDetails.DataSource = GetData(string.Format("select * from vw_PowderCoating where SubOA='{0}'", Id));
            gvDetails.DataBind();
        }
    }
