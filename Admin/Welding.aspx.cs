...
public partial class Admin_Welding : System.Web.UI.Page
{
    ...
    private void WeldingDDLbind()
    {
        SqlDataAdapter ad = new SqlDataAdapter("select WeldingId, OANumber as OANumber from tblWelding where IsApprove=1 and IsComplete is null order by WeldingId desc", con);
        DataTable dt = new DataTable();
        ad.Fill(dt);
        if (dt.Rows.Count > 0)
        {
            ...
            ddlONumber.DataValueField = "WeldingId";
            ddlONumber.DataBind();
    ...
    protected void GetWeldingData()
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                dgvWelding.DataSource = dt;
                dgvWelding.DataBind();
                ScriptManager.RegisterStartupScript(Page, this.GetType(), "Key", "<script>MakeStaticHeader('" + dgvWelding.ClientID + "', 900, 1020 , 40 ,true); </script>", false);
            }
            else
            {
                dgvWelding.DataSource = null;
                dgvWelding.DataBind();
                ScriptManager.RegisterStartupScript(Page, this.GetType(), "Key", "<script>MakeStaticHeader('" + dgvWelding.ClientID + "', 900, 1020 , 40 ,true); </script>", false);
    ...
    protected void GetSelectedRecords(object sender, EventArgs e)
    {
        ...
        if (Session["OneTimeFlag"] == null || Session["OneTimeFlag"].ToString() == "")
        {
            ...
            if (confirmValue == "Yes")
            {
                ...
                foreach (GridViewRow row in dgvWelding.Rows)
                {
                    if (row.RowType == DataControlRowType.DataRow)
                    {
                        CheckBox chkRow = (row.Cells[1].FindControl("chkRow") as CheckBox);
                        int totalCount = dgvWelding.Rows.Cast<GridViewRow>().Count(r => ((CheckBox)r.FindControl("chkRow")).Checked);
                        if (totalCount <= 0)
                        {
                        ...
                        else
                        {
                            if (chkRow.Checked)
                            {
                                ...
                                {
                                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Alert!!!!!- Outward Quantity Is Greater Than Inward Quantity...!');window.location.href='Welding.aspx';", true);
                                    return;
                                }
                ...
                using (con)
                {
                    using (SqlCommand cmd = con.CreateCommand())
                    {
                        ...
                        foreach (DataRow row in dt.Rows)
                        {
                            ...
                            SqlCommand cmdexsist = new SqlCommand("select OANumber,InwardQty,OutwardQty,SubOA from tblPowderCoating WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                            ...
                            SqlCommand cmd2 = new SqlCommand("select OutwardQty from tblWelding WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                            string Outward2Qty = "";
                            ...
                            if (OanumberExsists == "")
                            {
                                ...
                                using (SqlBulkCopy sqlBulkCopy = new SqlBulkCopy(con))
                                {
                                    ...
                                    sqlBulkCopy.DestinationTableName = "dbo.tblPowderCoating";
                            ...
                            else
                            {
                                ...
                                SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblPowderCoating] SET [InwardQty] = '" + totOutwardqnt.ToString() + "',[IsComplete]=NULL,[InwardDtTime]='" + row["outwarddatetime"].ToString() + "' WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                            ...
                            if (row["inwardqty"].ToString() == row["outwardqty"].ToString())
                            {
                                ...
                                //SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblWelding] SET [OutwardQty] = '" + row["totalinward"].ToString() + "',[InwardQty]='0',[IsComplete]=1,OutwardDtTime= '" + OutwardDtTime.ToString() + "',UpdatedDate='" + OutwardDtTime + "' WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                                SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblWelding] SET [OutwardQty] = '" + row["inwardqty"].ToString() + "',[InwardQty]='0',[IsComplete]=1,OutwardDtTime= '" + OutwardDtTime.ToString() + "',UpdatedDate='" + OutwardDtTime + "' WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                                cmdupdate.ExecuteNonQuery();
                            }
                            //END
                            else
                            {
                                ...
                                SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblWelding] SET [InwardQty] = '" + inwardqy.ToString() + "', [OutwardQty] = '" + totoutward.ToString() + "',OutwardDtTime= '" + OutwardDtTime.ToString() + "',UpdatedDate='" + OutwardDtTime + "' WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                                cmdupdate.ExecuteNonQuery();
                        ...
                        ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Successfully- Approved and send to Powder Coating Department...!');window.location.href='Welding.aspx';", true);
                        //Response.Redirect("Welding.aspx");
                    }
                }
            ...
            if (row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (chkRow.Checked == true)
                {
                    if (totalCount > 0)
                    {
                        if (InwardQty == 0)
                        {
                            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Less Quantity- You have sent Quantity to Powder Coating department...!')", true);
                        }
                        else
                        {
    ...
    protected void lnkbtnReturn_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                SqlCommand cmdselectt = new SqlCommand("select OutwardQty from tblCNCBending WHERE SubOA='" + hdnSubOANo.Value + "'", con);
                //Object Inwardqty = cmdselect.ExecuteScalar();
                ...
                if (Convert.ToInt32(txtReturnInward.Text) == Convert.ToInt32(hdnInwardQty.Value))
                {
                    ...
                    SqlCommand cmdupdate1 = new SqlCommand("UPDATE [dbo].[tblCNCBending] SET [InwardQty] = '" + TotalReturnInward + "',[IsComplete] = NULL, [OutwardQty] = '0' WHERE SubOA='" + hdnSubOANo.Value + "'", con);
                ...
                else
                {
                    ...
                    SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblWelding] SET [InwardQty] = '" + TotalReturn_Outward + "',[OutwardQty] = '" + TotalReturn_Outward + "' WHERE SubOA='" + hdnSubOANo.Value + "'", con);
                    cmdupdate.ExecuteNonQuery();
                ...
                con.Close();
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Quantity has been Return Successfully..!');window.location.href='Welding.aspx';", true);
            }
        }
        catch (Exception)
        {
    ...
    protected void dgvWelding_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "selectOAnumber")
        {
            ...
            if (oaNumber != "")
            {
                SqlCommand cmd = new SqlCommand("select OutwardQty from tblRptWelding WHERE SubOA='" + oaNumber + "'", con);
                string InwardQty = "";
                con.Open();
    ...
    protected void dgvWelding_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (Dt.Rows.Count > 0)
            {
                ...
                SqlDataAdapter Sdd = new SqlDataAdapter("Select * FROM   [ExcelEncLive].[tblUserRoleAuthorization] where UserID = '" + idd + "' AND PageName = 'Welding.aspx' AND PagesView = '1'", con);
                Sdd.Fill(Dtt);
                if (Dtt.Rows.Count > 0)
                {
                    dgvWelding.Columns[1].Visible = false;
                    dgvWelding.Columns[13].Visible = false;
            ...
            string Id = dgvWelding.DataKeys[e.Row.RowIndex].Value.ToString();
            GridView gvDetails = e.Row.FindControl("gvDetails") as GridView;
            gvDetails.DataSource = GetData(string.Format("select * from  [ExcelEncLive].[vw_Welding] where SubOA='{0}'", Id));
            gvDetails.DataBind();
        }
    }
    ...
    protected void btnPrintData_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            Response.Redirect("PDFShow.aspx?Name=Welding");
        }
        catch (Exception ex)
        {
            throw ex;
    ...
    {
        if (TextxtCustomerNametNew.Text == "")
        {
            string Report = "Welding";
        ...
        {
            string Report = "Welding";
            string Customer = Server.UrlEncode(TextxtCustomerNametNew.Text);
    ...
    protected void TextxtCustomerNametNew_TextChanged(object sender, EventArgs e)
    {
        FillGrid();
    }
    ...
    {
        SqlCommand cmdselect1 = new SqlCommand("select InwardQty from tblWelding WHERE SubOA='" + hdnSubOANo.Value + "'", con);
        Object LCInwardqty = cmdselect1.ExecuteScalar();

        SqlCommand cmdselect2 = new SqlCommand("select InwardQty from tblPowderCoating WHERE SubOA='" + hdnSubOANo.Value + "'", con);
        ...
        SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblWelding] SET [OutwardQty] = '" + CNCInwardQty + "' WHERE SubOA='" + hdnSubOANo.Value + "'", con);
        cmdupdate.ExecuteNonQuery();
        //END
        ...
        SqlCommand cmdupdatee = new SqlCommand("UPDATE [dbo].[tblCNCBending] SET [OutwardQty] = '" + LCInwardqty + "' WHERE SubOA='" + hdnSubOANo.Value + "'", con);
    ...
    /// <summary>
    /// 25-11-2024 changes by shubham  Wankhade 
    /// </summary>
    ...
    public void Getpreviousqty()
    {
        ...
        SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblWelding] SET [OutwardQty] = @OutwardQty WHERE SubOA='" + hdnSubOANo.Value + "'", con);
        ...
        SqlCommand cmdupdatee = new SqlCommand("UPDATE [dbo].[tblCNCBending] SET [OutwardQty] = '" + CNCInwardQty + "' WHERE SubOA='" + hdnSubOANo.Value + "'", con);
        cmdupdatee.ExecuteNonQuery();
        //END
    ...
    public void CheckInwardqtystages(String SubOA)
    {
        ...
        // Check tblWelding
        SqlCommand cmdselect4 = new SqlCommand("select OutwardQty from tblPowderCoating WHERE SubOA=@SubOA", con);
        ...
        // Now update the table with the determined CNCInwardQty
        SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblPowderCoating] SET [OutwardQty] = @OutwardQty WHERE SubOA=@SubOA", con);
        cmdupdate.Parameters.AddWithValue("@OutwardQty", CNCInwardQty);
    ...
    public void Checkoutwardqtnyofcuurentstage(String SubOA)
    {
        ...
        if (result != DBNull.Value && result != null && result.ToString() != "0")
        {
            ...
            SqlCommand cmdupdate1 = new SqlCommand("UPDATE [dbo].[tblWelding] SET [OutwardQty] = @OutwardQty WHERE SubOA=@SubOA", con);
