...
public partial class Admin_Welding_OldDB : System.Web.UI.Page
{
    ...
    {
        SqlDataAdapter ad = new SqlDataAdapter("select WeldingId, OANumber as OANumber from tblWeldingOld where IsApprove=1 and IsComplete is null order by WeldingId desc", con);
        DataTable dt = new DataTable();
        ad.Fill(dt);
        if (dt.Rows.Count > 0)
        {
    ...
    protected void GetWeldingData()
    {
        try
        {
            ...
            query = @"SELECT [WeldingId],[OANumber],[SubOA],[CustomerName],[Size],[TotalQty],[InwardDtTime],[InwardQty],[OutwardDtTime],[OutwardQty],
                ...
                FROM tblWeldingOld where IsComplete is null
            ...
            using (con)
            {
                using (SqlCommand cmd = con.CreateCommand())
                {
                    ...
                    foreach (DataRow row in dt.Rows)
                    {
                        ...
                        SqlCommand cmdexsist = new SqlCommand("select OANumber,InwardQty,OutwardQty,SubOA from tblPowderCoatingOldOld WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                        ...
                        SqlCommand cmd2 = new SqlCommand("select OutwardQty from tblWeldingOld WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                        string Outward2Qty = "";
                        ...
                        if (OanumberExsists == "")
                        {
                            ...
                            using (SqlBulkCopy sqlBulkCopy = new SqlBulkCopy(con))
                            {
                                ...
                                sqlBulkCopy.DestinationTableName = "dbo.tblPowderCoatingOldOld";
                        ...
                        else
                        {
                            ...
                            SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblPowderCoatingOldOld] SET [InwardQty] = '" + totOutwardqnt.ToString() + "',[IsComplete]=NULL,[InwardDtTime]='" + row["outwarddatetime"].ToString() + "' WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                            cmdupdate.ExecuteNonQuery();
                        }

                        if (row["inwardqty"].ToString() == row["outwardqty"].ToString())
                        {
                            SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblWeldingOld] SET [OutwardQty] = '" + row["totalinward"].ToString() + "',[InwardQty]='0',[IsComplete]=1,UpdatedDate='" + UpdatedDate + "' WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                            cmdupdate.ExecuteNonQuery();
                        }
                        else
                        {
                            ...
                            SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblWeldingOld] SET [InwardQty] = '" + inwardqy.ToString() + "', [OutwardQty] = '" + totoutward.ToString() + "',UpdatedDate='" + UpdatedDate + "' WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                            cmdupdate.ExecuteNonQuery();
                        }
                        con.Close();
                    }
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Successfully- Approved and send to Powder Coating Department...!');window.location.href='Welding_OldDB.aspx';", true);
    ...
    protected void lnkbtnReturn_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                //Get Exsiting Record
                SqlCommand cmdselect = new SqlCommand("select InwardQty from tblCNCBendingOldOld WHERE SubOA='" + hdnSubOANo.Value + "'", con);
                Object Inwardqty = cmdselect.ExecuteScalar();

                if (Convert.ToInt32(txtReturnInward.Text) == Convert.ToInt32(hdnInwardQty.Value))
                {
                    // If all record return
                    SqlCommand cmdDelete = new SqlCommand("Delete from [tblWeldingOld] WHERE SubOA='" + hdnSubOANo.Value + "'", con);
                    cmdDelete.ExecuteNonQuery();

                    int TotalReturnInward = Convert.ToInt32(Inwardqty.ToString()) + Convert.ToInt32(txtReturnInward.Text);
                    SqlCommand cmdupdate1 = new SqlCommand("UPDATE [dbo].[tblCNCBendingOldOld] SET [InwardQty] = '" + TotalReturnInward + "',[IsComplete] = NULL WHERE SubOA='" + hdnSubOANo.Value + "'", con);
                ...
                else
                {
                    ...
                    SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblWeldingOld] SET [InwardQty] = '" + TotalReturn_Outward + "',[OutwardQty] = '" + TotalReturn_Outward + "' WHERE SubOA='" + hdnSubOANo.Value + "'", con);
                    cmdupdate.ExecuteNonQuery();
                ...
                con.Close();
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Quantity has been Return Successfully..!');window.location.href='Welding_OldDB.aspx';", true);
            }
        }
        catch (Exception)
        {
    ...
    protected void dgvWelding_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (Dt.Rows.Count > 0)
            {
                ...
                SqlDataAdapter Sdd = new SqlDataAdapter("Select * FROM tblUserRoleAuthorization where UserID = '" + idd + "' AND PageName = 'Welding_OldDB.aspx' AND PagesView = '1'", con);
                Sdd.Fill(Dtt);
                if (Dtt.Rows.Count > 0)
                {
            ...
            gvDetails.DataSource = GetData(string.Format("select * from vw_WeldingOld where SubOA='{0}'", Id));
            gvDetails.DataBind();
        }
    }
